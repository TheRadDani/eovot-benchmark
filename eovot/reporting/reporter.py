...
class BenchmarkReporter:
    ...
    * **Console** — formatted summary block printed to stdout.
    ...
    def to_markdown_row(result: Dict[str, Any]) -> str:
        ...
        Columns: Tracker | Dataset | mIoU | Success AUC | Prec AUC | nPrec AUC | FPS | Latency (ms) | Mem (MB)
        ...
        nprec_auc = float(s.get("normalized_precision_auc", 0.0))
        ...
        return (
            ...
            f"| {success_auc:.4f} | {precision_auc:.4f} | {nprec_auc:.4f} "
    ...
    def comparison_table(results: List[Dict[str, Any]]) -> str:
        ...
        Includes standard VOT scalars (mIoU, success AUC, precision AUC,
        normalized precision AUC) alongside hardware metrics (FPS, latency,
        memory).  The nPrec AUC column aligns with the primary precision
        scalar reported by GOT-10k, VOT 2020+, and TrackingNet.
        ...
        """
        header = (
            "| Tracker | Dataset | mIoU | Success AUC | Prec AUC | nPrec AUC | FPS | Latency (ms) | Mem (MB) |\n"
            "|---------|---------|-----:|------------:|---------:|----------:|----:|-------------:|---------:|\n"
        )
