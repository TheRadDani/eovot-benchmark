...
class SequenceResult:
    """Per-sequence evaluation output.

    Attributes:
        sequence_name:    Identifier for the sequence.
        ious:             Per-frame IoU values, shape ``(N,)``.
        profiling:        Latency / memory summary from :class:`~eovot.profiling.profiler.Profiler`.
        ...
        energy:           CPU energy estimate; ``None`` when TDP profiling is disabled.
    """
...
class BenchmarkResult:
    """Aggregate evaluation output for one tracker on one dataset.

    Attributes:
        tracker_name:     Name of the evaluated tracker.
        dataset_name:     Label for the dataset used.
    ...
    def to_dict(self) -> Dict:
        ...
        * ``"sequences"`` — list of per-sequence metric dicts.
        ...
        for sr in self.sequence_results:
            ...
            if sr.mean_center_distance is not None:
                entry["mean_center_distance_px"] = round(sr.mean_center_distance, 3)
...
class BenchmarkEngine:
    ...
    Args:
        ...
        tdp_watts: If provided, enables CPU energy estimation via
            :class:`~eovot.profiling.energy.EnergyProfiler` using this TDP
            value in Watts.  Set to the device's CPU TDP for meaningful
            estimates (e.g. ``6.0`` for Raspberry Pi 4, ``15.0`` for a
            laptop).  Default: ``None`` (energy profiling disabled).

    Example::

        engine = BenchmarkEngine(verbose=True, tdp_watts=15.0)
    ...
    def run(
        ...
        Args:
            tracker:       Tracker instance conforming to :class:`~eovot.trackers.base.BaseTracker`.
            dataset:       Dataset yielding :class:`~eovot.datasets.base.Sequence` objects.
            dataset_name:  Human-readable label used in reports.
            max_sequences: Evaluate only the first N sequences (``None`` = all).
