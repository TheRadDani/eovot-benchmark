...
class SequenceResult:
    ...
    energy: Optional[EnergyResult] = None          # per-sequence energy estimate (None if disabled)
...
class BenchmarkResult:
    ...
    def summary(self) -> Dict:
        ...
        e_j = self.total_energy_j
        if e_j is not None:
            d["total_energy_j"] = round(e_j, 6)
        e_mj = self.mean_energy_per_frame_mj
        if e_mj is not None:
            d["mean_energy_per_frame_mj"] = round(e_mj, 4)
        return d

    def to_dict(self) -> Dict:
        """Serialize the full result to a dict compatible with BenchmarkReporter.

        Returns a nested dict with keys ``"summary"`` (aggregate metrics) and
        ``"sequences"`` (per-sequence breakdown), suitable for JSON export and
...
class BenchmarkEngine:
    ...
    Args:
        ...
        tdp_watts: If provided, enables CPU energy estimation using
            ...
            estimates (e.g. ``6.4`` for Raspberry Pi 4, ``15.0`` for a
