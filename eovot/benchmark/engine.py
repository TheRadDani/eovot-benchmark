...
class SequenceResult:
    ...
    def norm_precision_auc(self) -> Optional[float]:
        """Normalised AUC of the normalized precision curve (LaSOT standard), or None."""
        return self.accuracy_metrics.norm_precision_auc if self.accuracy_metrics else None
...
class BenchmarkResult:
    ...
    def mean_norm_precision_auc(self) -> Optional[float]:
        """Mean normalized precision AUC (LaSOT standard) across all sequences, or ``None``."""
        ...
        for seq in d.get("sequences", []):
            ...
            if "success_auc" in seq:
                accuracy = AccuracyMetrics(
                    ...
                    norm_precision_auc=float(seq.get("norm_precision_auc", 0.0)),
                )
